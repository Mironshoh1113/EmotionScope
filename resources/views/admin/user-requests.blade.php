    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.user_requests') }}</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $requests->where('response_code', '>=', 200)->where('response_code', '<', 300)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $requests->where('response_code', '>=', 400)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ round($requests->whereNotNull('response_time')->avg('response_time') ?? 0, 2) }}ms</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">{{ __('admin.all_requests') }}</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($requests as $request)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="showRequestDetails({{ $request->id }}, {{ $request->project_id }})" class="text-blue-600 hover:text-blue-900">
                                        {{ __('admin.view') }}
                                    </button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 text-center text-gray-500">{{ __('admin.no_requests_found') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-6">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        {{ __('admin.showing') }} {{ $requests->firstItem() }} {{ __('admin.to') }} {{ $requests->lastItem() }} {{ __('admin.of') }} {{ $requests->total() }} {{ __('admin.entries') }}
                    </div>
                    <div class="flex space-x-2">
                        @if($requests->onFirstPage())
                            <span class="px-3 py-2 text-sm text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">{{ __('admin.previous') }}</span>
                        @else
                            <a href="{{ $requests->previousPageUrl() }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{{ __('admin.previous') }}</a>
                        @endif
                        
                        @if($requests->hasMorePages())
                            <a href="{{ $requests->nextPageUrl() }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{{ __('admin.next') }}</a>
                        @else
...
<script>
function showRequestDetails(requestId, projectId) {
    ...
    fetch(`/admin/api/projects/${projectId}/requests/${requestId}`)
